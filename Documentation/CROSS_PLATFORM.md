...
Джойстик — **floating/dynamic**: он перехватывает касание по **всему экрану**, появляется в
точке нажатия и исчезает при отпускании. При `Awake` компонент сам растягивает свой
`RectTransform` на весь экран (прозрачная зона перехвата), создаёт видимые «фон-круг +
рукоятка» и прячет их, пока палец не нажат. Поэтому достаточно простого prefab:
...
2. Под Canvas создайте пустой GameObject **`VirtualJoystick`** с `RectTransform`.
3. Добавьте на него компонент **`ShelfRush.UI > VirtualJoystick`**.
4. (Опционально) добавьте **`Image`** — цвет будет принудительно сделан прозрачным в
   `Awake` (это полноэкранная зона перехвата касаний), `RaycastTarget` остаётся включён.
5. (Опционально) назначьте `backgroundSprite` / `handleSprite` и существующий `Handle`.
   Если `Handle` не назначен — рукоятка создаётся автоматически.
6. Настройте `dragRadius` (радиус хода рукоятки от точки нажатия), `deadZone`,
   `baseSize`/`handleSize`, `showOnlyWhileHeld`.
7. Сохраните как prefab: перетащите объект из Hierarchy в `Assets/Prefabs/UI/`.

> Работает из любой точки экрана: круг появляется под пальцем. Перехват идёт через
> прозрачный полноэкранный root (RaycastTarget включён), поэтому джойстик не конфликтует
> по позиции — но он перехватывает касания везде. Если нужно оставить кнопки/элементы
> HUD кликабельными — расположите их ВЫШЕ джойстика по `sibling index` (позже в иерархии
> Canvas) или сделайте у кнопок собственный `RaycastTarget`.
